...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        if not partner_id.credit_check:
            if not self.account_payment_ids:
                raise AccessDenied(_('You Can not Confirm sale order without any payment'))
        # total_amount = self.amount_total
        # total_amount = sum(self.env['sale.order'].search_read([('partner_id', '=', self.partner_id.id), ('state', '=', 'sale')], ['amount_total'])) + self.amount_total
        if partner_id.credit_check:
            ...
            if partner_id.credit_blocking <= total_amount and not existing_move:
                raise AccessDenied(_('Customer credit limit exceeded without having a recievable'))
