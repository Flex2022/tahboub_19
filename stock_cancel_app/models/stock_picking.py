...
class StockPicking(models.Model):
    ...
    def picking_draft_from_code(self, picking_id):
        stock_id = picking_id
        for rec in stock_id:
            rec.with_context({'Flag': True}).action_cancel()
            rec.state = 'draft'
    ...
    def picking_draft_back_end(self):
        if self.user_has_groups('stock_cancel_app.group_show_cancel_stock_picking'):
            stock_id =self
            for rec in stock_id:
                rec.with_context({'Flag':True}).action_cancel()
            ...
            if self.env.context.get('Flag',False) and picking.state =='done':
                ...
                for move in account_moves:
                    ...
                    try:
                        ...
                        pass
                    if landed_cost_rec:           
                        raise exceptions.UserError('This Delivery is set in landed cost record %s you need to delete it fisrt then you can cancel this Delivery' % ','.join(landed_cost_rec.mapped('name')))
